...
def call_gemini(user_data: dict) -> dict:
    ...
    if is_dating:
        analysis_context = (
            "현재 파트너가 있는(연애 중 또는 결혼/동거) 사용자입니다. "
            "현재 파트너가 운명의 상대인지 분석하고, "
            "현재 관계의 방향성과 조언을 중심으로 답해주세요. "
            "배우자 프로필은 '현재 파트너에게 가장 어울리는 이상적 파트너상'으로 해석해주세요. "
            "현재 파트너와 처음 만났던 시기나 계기에 대한 언급은 가능하지만, "
            "그 이전의 과거사(과거 연애, 과거 운세 등)는 절대 언급하지 마세요. "
            "또한 새로운 사람을 만나는 시나리오는 쓰지 마세요. 이미 파트너가 있습니다."
        )
        meeting_context = (
            "meeting_prediction의 timing은 '현재 연인과의 관계가 한 단계 발전하는 시기', "
            "place는 '관계 전환점이 되는 장소', "
            "first_meet_scenario는 '현재 연인과의 관계에서 가장 결정적인 순간 시나리오', "
            "first_conversation은 '연인과의 관계 전환점에서의 대화'로 작성해주세요."
        )
    else:
        analysis_context = "현재 싱글인 사용자입니다. 미래에 만날 운명의 배우자를 예측해주세요."
        meeting_context = "meeting_prediction은 미래 배우자와의 첫 만남을 예측해주세요."

    dating_warning_context = "파트너가 있는 사람(연애 중 또는 결혼/동거)에게 맞는 조언을 해줘." if is_dating else "솔로에게 맞는 조언을 해줘."
    timing_label = "관계가 더 깊어지는 시기" if is_dating else "만나는 시기"
...
## ⏰ 시간 기준 규칙 (반드시 준수)
- 오늘 날짜: {today_str}
- 모든 예측과 조언은 반드시 현재 시점({current_year}년) 이후의 미래만 다뤄주세요.
- 파트너가 있는 사용자의 경우, 현재 파트너와의 첫 만남 시기/계기는 언급할 수 있습니다.
- 그 외 과거에 대한 회고, "이미 ~했을 것", "과거에 ~", "작년에 ~" 등 과거 시점 서술은 절대 금지합니다.
- love_timeline은 반드시 {current_year}년부터 시작해주세요.
...
- 연애 상태: {"파트너 있음 (연애 중 또는 결혼/동거)" if is_dating else "싱글"}
...
def render_basic_info():
    ...
    with col2:
        if st.button("다음 →", use_container_width=True):
            ...
            st.session_state.user_data["is_dating"] = (is_dating == "네, 파트너가 있어요 (연애/결혼)")
...
def render_result():
    ...
    # 히어로 태그라인
    # ══════════════════════════════════════
    dating_badge = "💑 파트너 있음" if is_dating else "💫 솔로"
